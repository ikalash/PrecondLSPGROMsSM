...
calc_traction_components(Kokkos::DynRankView<ScalarT, PHX::Device> & qp_data_returned,
  ...
  int numCells = qp_data_returned.extent(0); // How many cell's worth of data is being computed?
...
calc_gradu_dotn_const(Kokkos::DynRankView<ScalarT, PHX::Device> & qp_data_returned,
  ...
  int numCells = qp_data_returned.extent(0); // How many cell's worth of data is being computed?
  int numPoints = qp_data_returned.extent(1); // How many QPs per cell?
  int numDOFs = qp_data_returned.extent(2); // How many DOFs per node to calculate?
